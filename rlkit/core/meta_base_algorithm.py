...
class MetaBaseAlgorithm(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if replay_buffer is None:
            replay_buffer = MetaEnvReplayBuffer(
                replay_buffer_size_per_task, env, random_seed=np.random.randint(10000)
            )
        else:
    ...
    def do_task_rollout(self, task_params, num_steps):
        ...
        while len(self._current_path_builder) < self.max_path_length:
            ...
            if self.render:
                self.training_env.render(self.render_kwargs)

            next_obs, raw_reward, terminal, env_info = self.training_env.step(action)
            if self.no_terminal:
                terminal = False
            ...
            self._handle_step(
                ...
                absorbing=np.array([0.0, 0.0]),  # not implemented wrap absorbing
