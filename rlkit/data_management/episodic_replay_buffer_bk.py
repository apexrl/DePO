...
class EpisodicReplayBuffer(ReplayBuffer):
    ...
    def get_sufficient_buffer(
        self, benchmark_rew=150, ratio=0.6, random_benchmarks=150, value_func=None
    ):
        """
        ...
        if type(benchmark_rew) == int:
            selected_trajs = [
                ...
                if (
                    self._data[ind]["ep_discount_rews"]
                    + last_pair["gamma_pow"][ind] * last_q[ind]
                )
                <= benchmark_rew
            ]
        else:
        ...
        if len(selected_trajs) <= 0:
            if benchmark_rew <= random_benchmarks:
                logger.log(
                    "Use the pretraining dataset! Current evaluation {}".format(
                        np.mean(benchmark_rew)
                    )
                )
                return self.get_pretrain_buffer(random_benchmarks)
            else:
                logger.log(
                    "Higher than the dataset! Current evaluation {}".format(
                        np.mean(benchmark_rew)
                    )
                )
                ...
                selected_trajs = [
                    _
                    for _ in self._data
                    if ratio <= _["ep_rews"] / np.mean(benchmark_rew) <= 1 / ratio
                ]
        ...
        # traj_num = min(10, len(selected_trajs))

        raw_len = len(selected_trajs)
        selected_trajs = selected_trajs[:traj_num]

        logger.log(
            "Current evaluation: {}, Sufficient num: {}, Selected num: {}, Use num: {}".format(
                np.mean(benchmark_rew),
                raw_len,
                traj_num,
                self.simple_buffer.get_traj_num(),
            )
        )
        for traj in selected_trajs:
            self.simple_buffer.add_path(traj)
