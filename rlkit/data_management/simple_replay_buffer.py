...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        if isinstance(observation_dim, tuple):
            ...
            self._pred_obs = np.zeros(dims, dtype=obs_dtype)
            self._action_log_probs = np.zeros(1, dtype=np.float64)
            ...
            self._observations = np.zeros(
                (max_replay_buffer_size, observation_dim), dtype=obs_dtype
            )
            self._next_obs = np.zeros(
                (max_replay_buffer_size, observation_dim), dtype=obs_dtype
            )
            self._pred_obs = np.zeros(
                (max_replay_buffer_size, observation_dim), dtype=obs_dtype
            )
            self._action_log_probs = np.zeros(
                (max_replay_buffer_size, 1), dtype=np.float64
            )
        ...
        **kwargs
        ...
        if isinstance(self._observations, dict):
            ...
            if action_log_prob is not None:
                self._action_log_probs[key][self._top] = action_log_prob
            ...
            if action_log_prob is not None:
                self._action_log_probs[self._top] = action_log_prob
        self._advance()
    ...
    def save_data(self, save_name):
        save_dict = {
            ...
            "action_log_probs": self._action_log_probs[: self._top],
    ...
    def _get_batch_using_indices(
        ...
        if "action_log_probs" in keys:
            ret_dict["action_log_probs"] = pred_obs_to_return
